...
class Vue():
    ...
    def frameLoadProjet(self):
        ...
        lnp = Label(self.frameLoadProjet, text=Projet[0][1])
        lnp.grid(sticky=W, column=1, row=1)
        print(Projet[0][1])
        Membres = self.parent.modele.afficherMembres(str(projetSelecter[1]))
        lnm = Label(self.frameLoadProjet, text=Membres)
        lnm.grid(sticky=W, column=1, row=2)
        print(Membres)
        Etapes = self.parent.modele.afficherEtapes(str(projetSelecter[1]))
        le = Label(self.frameLoadProjet, text=Etapes)
        le.grid(sticky=W, column=1, row=3)
        print(Etapes)
        Sprints = self.parent.modele.afficherSprints(str(projetSelecter[1]))
        ls = Label(self.frameLoadProjet, text=Sprints)
        ls.grid(sticky=W, column=1, row=4)
        print(Sprints)
        Taches = self.parent.modele.afficherTaches(str(projetSelecter[1]))
        lt = Label(self.frameLoadProjet, text=Taches)
        lt.grid(sticky=W, column=1, row=5)
        print(Taches)
